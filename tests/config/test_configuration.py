...
class TestConfiguration:
    def setup_class(self):
        os.environ["SITE_NAME"] = "local"
        os.environ["SERIAL_NUMBER"] = "123XX"

    def test_throws_exception_when_configuration_file_not_found(self):
        with pytest.raises(ConfigurationFileNotFoundError):
            Configuration.from_file("./tests/config/bad-example_configuration.yaml")

    def test_throws_exception_when_configuration_file_has_syntax_errors(self):
    ...
    def test_get_holding_registers(self):
        configuration = Configuration.from_file(_config_path())
    ...
    def test_able_to_get_mqtt_settings(self):
        configuration = Configuration.from_file(_config_path())
    ...
    def test_able_to_get_modbus_settings(self):
        configuration = Configuration.from_file(_config_path())
    ...
    def test_validate_yaml_data(self):
        ...
        try:
            _validate_yaml_data(without_error_topic)
        except Exception:
            assert False
        ...
        for char in ("#", "+"):
            ...
            with pytest.raises(ConfigurationFileInvalidError) as ex:
                _validate_yaml_data(error_topic_wildcard)
    ...
    def test_key_error_in_config_parsing(self, monkeypatch):
        """
